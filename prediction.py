...
def prediction():
    ...
    for name, group in group1:
        if(len(group)>=4): # 本系统数据无法解决的情况舍去
            ...
            if((group.iloc[0,1].to_pydatetime()+dt.timedelta(days=pred))<dt.datetime.now()+dt.timedelta(days=10)): # 本系统数据无法解决的情况舍去
                continue
            if(disaster_time>(group.iloc[0,1].to_pydatetime()+dt.timedelta(days=pred))): # 比较时间先后
                ...
                disaster_name = name
    return str(disaster_time),disaster_name


def exp_smooth(list,a): # 指数加权平滑法
    result=0
    for i in range(len(list)):
        result += a*(1-a)**(i)*list[i]
    return result
